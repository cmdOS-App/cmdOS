...
const DEFAULT_CHOICES = [
  ...
  { name: 'New Tab Override', value: 'AltS_search_newtab' },
...
export default async function deleteModules(manifestObject: chrome.runtime.ManifestV3) {
  ...
  if (answers.includes('AltS_search_newtab')) {
...
async function deleteNewTabOverride(manifestObject: chrome.runtime.ManifestV3) {
  await zipFolder(resolve(pagesPath, 'AltS_search_newtab'), resolve(archivePath, 'AltS_search_newtab.zip'));
  void rimraf(resolve(pagesPath, 'AltS_search_newtab'));
  if (manifestObject.chrome_url_overrides) {
    delete manifestObject.chrome_url_overrides.AltS_search_newtab;
  }
}
