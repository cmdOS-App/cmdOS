...
function recoverNewTabOverride(manifestObject: chrome.runtime.ManifestV3) {
  manifestObject.chrome_url_overrides = {
    AltS_search_newtab: 'AltS_search_newtab/index.html',
  };
  const zipFilePath = path.resolve(archivePath, 'AltS_search_newtab.zip');
  upZipAndDelete(zipFilePath, path.resolve(pagesPath, 'AltS_search_newtab'));
}
